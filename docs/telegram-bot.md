...
|---|---|
| /price | Current prices for the sizes you watch (or sensible defaults). Sizes quoted by several sources (Antam) get their own block right in the compact view — the official Logam Mulia price pinned first with a 🏛 badge, the rest cheapest first, cheapest tagged — and a button expands to the full board: every size of every source, one block per source |
...
- By source (brands with 2+ live sources, i.e. Antam): the same size quoted
  by every source that responded — official pinned first with a 🏛 badge, the
  rest cheapest first with their premium over the cheapest — plus which
  source pays the best buyback today. The verdict
